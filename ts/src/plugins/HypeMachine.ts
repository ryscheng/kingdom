...
class HypeMachine implements Plugin {
   ...
   **/
  constructor(audioOut: AudioOut, username?: string) {
    // Private variables
    this.log = winston.loggers.get("plugins");
    this._audioOut = audioOut; // driver
    this._username = username ?? DEFAULT_PLAYLIST; // hypem.com username
    this._cachedResult = {}; // caches results from hypem API
    this._queries = {
      // list of hypejs calls
    ...
    this.intents = [
      {
        ...
        parameters: [{ name: "Playlist", type: "HYPEM_PLAYLISTS" }],
        utterances: ["queue *Playlist hype"],
      },
    ];
    this.types = {
  ...
  queue(playlistName: string) {
    ...
    if (
      !this._queries.hasOwnProperty(playlistName) ||
      playlistName === null ||
      ...
      this.log.verbose(
        "HypeMachine.queue() defaulting to default, my favorites",
      );
    }
    ...
    if (
      Array.isArray(this._cachedResult[playlistName]) &&
      this._cachedResult[playlistName].length > 0
    ) {
      this.log.verbose(
        "HypeMachine.queue() has cached playlist. Sending " +
          this._cachedResult[playlistName].length +
          " songs to AudioOut",
      );
    ...
    return Q.nfapply(ref, ["0"]).then((result) => {
      ...
      for (let i = 0; i < result.length; i++) {
        if (
          ...
          result[i].hasOwnProperty("tracks")
        ) {
